...
const { witels, rtus, groupUsers, picUsers, oldAlarms, jobQueueNumber } = workerData;
...
const fetchPortSensor = async (witelId) => {
    console.log("Create newosase api request", { witelId });
    try {

        // const params = { isAlert: 1, witelId };
        const params = { searchRtuSname: "RTU00-D7-MAT", searchNoPort: "A-40" };
        ...
        if(response.data && response.data.result && response.data.result.payload) {
            console.log("Collecting newosase api response", { witelId });
            return response.data.result.payload.map(item => {
                if(item.rtu_sname == "RTU00-D7-MAT" && item.no_port == "A-40") {
                    item.severity.name = "Warning";
                }
                return item;
            });
        }

        console.log(`No data provided in ${ newosaseBaseUrl }/getrtuport`, {
            witelId,
        ...
        console.log(`Error in ${ newosaseBaseUrl }/getrtuport`, { witelId });
        console.error(err);
        return [];
...
const defineAlarm = async ({ witel, rtus, groupUsers, picUsers, oldAlarms }) => {
    ...
    console.log("Get witel's old alarm", {
        witelId: witel.id,
    ...
    console.log("Split alarm status category", {
        witelId: witel.id,
...
console.log(`Starting port-alarm thread, queue:${ jobQueueNumber }, witelIds:(${ witelIdsStr })`);
